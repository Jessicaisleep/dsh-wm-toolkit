/**
 * dsh-wm-toolkit — 浏览器半边（**生成物**：改 parts/ 后运行 node build.mjs，勿直接编辑本文件）。
 *
...
/** 界面文案（单语）。原三语字典的英文/日文值本就是中文，多语机制已移除。 */
    var TEXT = {
      ...
      checking: "检查中…",
      upToDate: "已是最新版本",
      localBuild: "本地构建（无更新通道）：改源码后重建并同步部署副本即可",
      ...
      imageOriginal: "原图预览",
      removeImage: "移除图片",
      // ---- WM 分叉新增：编辑「我的回复」（助手消息）----
      wmEditAssistant: "编辑这条回复",
      wmEditTitle: "编辑我的回复",
      wmEditHint: "与「编辑我的消息」同一机制：确定后会归档当前会话并开一个新分支，把这个回合的回复改成新文本。这个回合之后的内容留在旧会话里（可从版本切换 < > 回去）。",
      wmEditWorking: "正在生成新分支…",
      wmEditWriting: "正在写入新文本…",
      wmEditOpening: "正在打开新分支…",
      wmEditEmpty: "回复内容不能为空",
      wmEditNoText: "这条回复没有可编辑的文本",
      wmEditTurnOpen: "这个回合还没结束，等回复完成后再编辑",
      wmEditOpenFail: "生成新分支失败，已保留原会话",
      wmEditWriteFail: "写入新文本失败（分支已建好但仍是旧文本）",
      wmEditSessionGone: "当前会话不在宿主里（可能已归档），请先切回该会话再编辑",
      wmEditLiveWarn: "分支已被宿主加载，已请求重新读取；若仍显示旧文本，切到别的会话再回来即可",
      wmEditDone: "已在新分支中改好这条回复",
      wmTabText: "正文",
      wmTabReasoning: "思考",
      wmRegenerate: "用原提问重新生成",
      wmRegenerateHint: "会开一个新分支，用这条回复对应的原提问重跑一遍；这一回合之后的对话留在旧会话里。",
      wmRegenerating: "正在重跑…",
      wmRegenerateDone: "已在新分支重新生成（原提问已自动重发）",
      wmRegenerateNoInput: "这个回合没有可重放的用户输入",
      wmEditCancelShort: "取消（Esc）",
      wmEditConfirmShort: "确定（Ctrl+Enter）",
    };
    ...
    // ================= WM 分叉新增：编辑「我的回复」 =================
    // 机制与「编辑我的消息」完全一致（归档旧会话 + 开新分支），只多一步改写：
    //   1. 宿主解出边界 = 目标回合自己的 turn/end（子会话保留该回合，含这条回复）；
    //   2. client 用官方 ctxSessions.fork 在该边界开分支（子会话身份先铸好并预登记关系）；
    //   3. 子会话此刻是冷态的，宿主直接把那条 assistant/message 的 text 块改写成新文本；
    //   4. 受控切换打开子会话，成功后归档父会话（= 旧版本），成员进同一版本家族（< > 可回）。
    // 该回合之后的内容随父会话一起成为旧版本。

    // 诊断计数器：只为把"按钮到底有没有渲染、槽位到底给了什么 props"写进宿主日志，便于排障。
    var wmAssistDiag = { renders: 0, logged: 0 };

    /**
     * 目标解析一律走宿主：chat 槽位只给 messageId，而宿主手上的会话事件里
     * `data.message.id` 与投影的 `finalNode.messageId` 同源（实测 2936/2936 命中），
     * 原文也由宿主一并给出——这样客户端完全不依赖投影 block 的形状
     * （投影用 `kind`，事件日志用 `type`，两者不同，曾因此取不到文本）。
     */
    function resolveAssistantTarget(sessionId, messageId, hint) {
      var payload = { sessionId: sessionId, messageId: messageId };
      if (hint && typeof hint.seq === "number") payload.targetSeq = hint.seq;
      if (hint && typeof hint.turn === "number") payload.turn = hint.turn;
      return fetch("/bubble/recall-assistant", {
        method: "POST",
        headers: { "content-type": "application/json" },
        ...
        var d = res.d;
        if (!d.ok) {
          var code = d.error || "unknown";
          var msg = code === "turn-open" ? TEXT.wmEditTurnOpen
            : (code === "no-text" || code === "no-assistant-text") ? TEXT.wmEditNoText
              : code === "session-not-found" ? TEXT.wmEditSessionGone
                : (d.message || TEXT.wmEditOpenFail);
    ...
    /**
     * 按元素**实际渲染出来的背景亮度**选前景色。
     *
     * 为什么不能只写 var(--dsw-alias-label-primary)：这套主题下部分 alias 变量并不存在，
     * 于是背景退回深色兜底、文字却是主题给的深色 → 黑字黑底（"全黑底提示啥也看不见"）。
     * 读一次计算样式即可与任何主题自洽：浅底给深字，深底给浅字。
     */
    function wmReadableTextColor(el) {
      try {
        var bg = getComputedStyle(el).backgroundColor || "";
        var m = /rgba?\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)(?:[,\s\/]+([\d.]+))?/.exec(bg);
        var alpha = m && m[4] !== undefined ? parseFloat(m[4]) : 1;
        if (!m || alpha <= 0.2) return "#f2f2f2";
        var lum = 0.299 * Number(m[1]) + 0.587 * Number(m[2]) + 0.114 * Number(m[3]);
        return lum > 150 ? "#141414" : "#f2f2f2";
      } catch (e) { return "#f2f2f2"; }
    }

    /** 轻量提示条（无第三方依赖，自动消失）。 */
    function wmToast(text, level) {
      try {
        var el = document.createElement("div");
        el.setAttribute("data-dsh-message-recall", "wm-toast");
        el.textContent = text;
        el.style.cssText = "position:fixed;left:50%;bottom:36px;transform:translateX(-50%);z-index:2147483200;"
          + "max-width:min(680px,92vw);padding:9px 14px;border-radius:9px;font-size:13px;line-height:19px;"
          + "box-shadow:0 8px 28px rgba(0,0,0,.35);transition:opacity .25s ease;"
          + "background:var(--dsw-alias-bg-layer-3, #2b2b2b);border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.35));";
        if (level === "error") el.style.borderColor = "var(--dsw-alias-state-error-primary, #d97706)";
        document.body.appendChild(el);
        // 插进去之后才知道背景到底渲染成什么色 → 此刻再定前景色（并给个描边提升可读性）
        var fg = wmReadableTextColor(el);
        el.style.color = fg;
        el.style.textShadow = fg === "#f2f2f2" ? "0 1px 2px rgba(0,0,0,.45)" : "none";
        setTimeout(function () { el.style.opacity = "0"; }, 2600);
        setTimeout(function () { try { el.remove(); } catch (eR) { /* ignore */ } }, 3000);
      } catch (e) { /* 提示失败不影响主流程 */ }
    }
    /**
     * 自绘编辑对话框（Electron 不支持 window.prompt/confirm）。
     * onConfirm(newText) 返回 Promise；resolve(true) 关闭，resolve(false) 保留窗口以便重试。
     */
    function openAssistantEditDialog(opts) {
      var old = document.querySelector('[data-dsh-message-recall="wm-edit-dialog"]');
      if (old) { try { old.remove(); } catch (eOld) { /* ignore */ } }

      var overlay = document.createElement("div");
      overlay.setAttribute("data-dsh-message-recall", "wm-edit-dialog");
      overlay.style.cssText = "position:fixed;inset:0;z-index:2147483100;background:rgba(0,0,0,.42);"
        + "display:flex;align-items:center;justify-content:center;padding:24px;";

      var card = document.createElement("div");
      card.style.cssText = "width:min(760px,94vw);max-height:82vh;display:flex;flex-direction:column;gap:10px;"
        + "padding:16px 16px 14px;border-radius:13px;box-shadow:0 16px 48px rgba(0,0,0,.45);"
        + "background:var(--dsw-alias-bg-layer-3, #232323);color:var(--dsw-alias-label-primary, #f1f1f1);"
        + "border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.32));font-size:13px;line-height:20px;";

      var title = document.createElement("div");
      title.textContent = TEXT.wmEditTitle;
      title.style.cssText = "font-size:15px;font-weight:600;";
      card.appendChild(title);

      var hint = document.createElement("div");
      hint.textContent = TEXT.wmEditHint;
      hint.style.cssText = "font-size:12px;line-height:18px;color:var(--dsw-alias-label-tertiary, #9a9a9a);";
      card.appendChild(hint);

      // 正文 / 思考 切换（仅当该消息带思考块时出现）——借鉴 dsh-message-edit 的"可编辑块"思路
      var blocks = { text: typeof opts.text === "string" ? opts.text : "" };
      if (typeof opts.reasoningText === "string" && opts.reasoningText.length > 0) blocks.reasoning = opts.reasoningText;
      var currentKind = "text";

      var ta = document.createElement("textarea");
      ta.value = blocks.text;
      ta.setAttribute("data-dsh-message-recall", "wm-edit-textarea");
      ta.style.cssText = "flex:1 1 auto;min-height:240px;max-height:56vh;overflow:auto;resize:vertical;padding:10px 12px;"
        + "border-radius:9px;font:inherit;font-size:13px;line-height:20px;white-space:pre-wrap;"
        + "background:var(--dsw-alias-fill-l2, rgba(128,128,128,.10));color:inherit;"
      ...
      if (blocks.reasoning !== undefined) {
        ...
        function syncTabs() {
          Object.keys(tabBtns).forEach(function (k) {
            var on = k === currentKind;
            tabBtns[k].style.background = on ? "var(--dsw-alias-fill-l2, rgba(128,128,128,.18))" : "transparent";
            tabBtns[k].style.fontWeight = on ? "600" : "400";
            tabBtns[k].style.opacity = on ? "1" : "0.72";
          });
        }
        ["text", "reasoning"].forEach(function (kind) {
          var b = document.createElement("button");
          b.type = "button";
          b.textContent = kind === "text" ? TEXT.wmTabText : TEXT.wmTabReasoning;
          b.style.cssText = "appearance:none;cursor:pointer;font:inherit;font-size:12px;padding:4px 12px;border-radius:999px;"
            + "border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.32));color:inherit;background:transparent;";
          b.addEventListener("click", function () {
            if (kind === currentKind) return;
            blocks[currentKind] = ta.value;
            currentKind = kind;
            ta.value = blocks[kind] || "";
            syncTabs();
          });
          tabBtns[kind] = b;
          tabs.appendChild(b);
      ...
      status.setAttribute("data-dsh-message-recall", "wm-edit-status");
      status.style.cssText = "min-height:19px;font-size:12px;color:var(--dsw-alias-label-secondary, #bdbdbd);";
      ...
      function mkBtn(label, primary) {
        ...
        b.type = "button";
        b.textContent = label;
        b.style.cssText = "appearance:none;cursor:pointer;font:inherit;font-size:13px;padding:7px 15px;border-radius:8px;"
          + (primary
            ? "border:1px solid transparent;background:var(--dsw-alias-bg-accent, #4d6bfe);color:#fff;font-weight:600;"
            : "border:1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.32));background:transparent;color:inherit;");
        b.addEventListener("mouseenter", function () { b.style.opacity = "0.88"; });
        b.addEventListener("mouseleave", function () { b.style.opacity = "1"; });
        return b;
      }
      var cancelBtn = mkBtn(TEXT.wmEditCancelShort, false);
      var okBtn = mkBtn(TEXT.wmEditConfirmShort, true);
      ...
      function close() {
        if (closed) return;
        closed = true;
        try { document.removeEventListener("keydown", onKey, true); } catch (eK) { /* ignore */ }
        try { overlay.remove(); } catch (eR2) { /* ignore */ }
      }
      function onKey(e) {
      ...
      function submit() {
        if (busy) return;
        var value = ta.value;
        if (value.trim().length === 0) { status.textContent = TEXT.wmEditEmpty; return; }
        busy = true;
        okBtn.disabled = true; cancelBtn.disabled = true;
        Promise.resolve(opts.onConfirm(value, currentKind)).then(function (done) {
          busy = false;
      ...
      // 主题自适应：按卡片实际背景亮度决定前景色。
      // 只用 --dsw-alias-* 变量在浅色主题/变量缺失时会退化成"白字白底"（取消按钮看不见），
      // 所以这里读一次计算样式，浅底给深字、深底给浅字，任何主题都可读。
      try {
        var fg = wmReadableTextColor(card);
        card.style.color = fg;
        ta.style.color = fg;
        hint.style.color = fg; hint.style.opacity = "0.72";
        status.style.color = fg; status.style.opacity = "0.8";
      } catch (eC) { /* 取不到就当深色主题 */ }
      try { ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); } catch (eF) { /* ignore */ }
      return { close: close, setStatus: function (t) { status.textContent = t; }, textarea: ta };
    }

    /**
     * 执行一次「编辑我的回复」：**让宿主用改好的种子直接建分支** → 登记关系 → 打开 → 归档旧会话。
     *
     * 为什么不用官方 fork 再事后改写：fork 出来的子会话在 fork 那一刻就已经是活动会话，
     * 而 DSH 的会话事件消息是 deepFreeze 的，磁盘改了内存也改不动（界面仍旧文本）；
     * 而"卸载活动会话"会把该行从运行时与客户端列表里摘掉，导致 sessions.select 报
     * unknown session（切换卡死、< > 失效）。所以改为：建分支时种子就带新文本。
     */
    function runAssistantEdit(ctxLike, sessionId, resolved, newText, ui, blockKind) {
      var setStatus = ui && typeof ui.setStatus === "function" ? ui.setStatus : function () {};
      // 记下父会话当前的模型/挡位：分支打开后写回子会话，避免新分支悄悄跑在默认档上。
      var parentSel = null;
      try { if (ctxLike.modelSel && typeof ctxLike.modelSel.capture === "function") parentSel = ctxLike.modelSel.capture(sessionId); } catch (eSel) { parentSel = null; }
      var expectedId = mintChildSessionId();
      var preRel = relationOfSession(sessionId);
      var rowId = preRel ? preRel.rowId : sessionId;
      var preMembers = preRel ? preRel.memberSessionIds.slice() : [sessionId];
      if (preMembers.indexOf(expectedId) < 0) preMembers.push(expectedId);

      // 先铸好子会话 id 并预登记关系：宿主直接用它建分支，子会话一出现就已被折叠进原条目。
      setStatus(TEXT.wmEditWorking);
      return registerRelation(rowId, expectedId, preMembers).then(function (preRegistered) {
        setStatus(TEXT.wmEditOpening);
        return fetch("/bubble/assistant-branch", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            parentSessionId: sessionId,
            childSessionId: expectedId,
            targetSeq: resolved.targetSeq,
            messageId: resolved.messageId,
            turn: resolved.turn,
            newText: newText,
            blockKind: blockKind === "reasoning" ? "reasoning" : "text",
            blockIndex: blockKind === "reasoning" ? resolved.reasoningIndex : resolved.blockIndex
          })
        }).then(function (r) { return r.json().then(function (d) { return { status: r.status, d: d || {} }; }); }).then(function (res) {
          var d = res.d;
          if (!d.ok) {
            if (preRegistered) {
              var keep = [];
              for (var i = 0; i < preMembers.length; i++) if (preMembers[i] !== expectedId) keep.push(preMembers[i]);
              if (keep.length === 0) keep.push(sessionId);
              try { registerRelation(rowId, sessionId, keep); } catch (eRb) { /* 残留无害 */ }
            }
            var code = d.error || "unknown";
            var msg = code === "turn-open" ? TEXT.wmEditTurnOpen
              : code === "no-text" || code === "no-assistant-text" ? TEXT.wmEditNoText
          ...
          return {
            newId: d.childSessionId || expectedId,
            expectedId: expectedId,
            rowId: rowId,
            preRegistered: preRegistered,
            seedEvents: d.seedEvents,
            attach: d.attach
          };
        });
      }).then(function (st) {
        // 关系核对（宿主采用了预分配 id 时是空操作）+ 兜底登记
        var chain = Promise.resolve(st.preRegistered === true);
        if (st.newId !== st.expectedId) chain = Promise.resolve(reconcileForkChild(st.rowId, st.expectedId, st.newId, sessionId));
        return chain.then(function (settled) {
          if (settled === true) return st;
          var rel = relationOfSession(sessionId);
          var members = rel ? rel.memberSessionIds.slice() : [sessionId];
          if (members.indexOf(st.newId) < 0) members.push(st.newId);
          return registerRelation(st.rowId, st.newId, members).then(function () { return st; });
        });
      }).then(function (st) {
        log("info", "wm-assistant", "分支已建好，开始打开", { parent: sessionId, child: st.newId, seedEvents: st.seedEvents, attach: st.attach });
        return controlledSwitch({
          opKey: "wmedit:" + sessionId + ">" + st.newId,
          sourceId: sessionId,
          targetId: st.newId,
          ctxSessions: ctxLike.ctxSessions,
          openSession: function (id) { try { ctxLike.openSession(id); } catch (eO) { /* ignore */ } },
          archiveSession: ctxLike.archiveSession,
          restoreSession: ctxLike.restoreSession,
          prepareSession: (ctxLike.ctxSessions && typeof ctxLike.ctxSessions.prepare === "function")
            ? function (id, signal) { return ctxLike.ctxSessions.prepare(id, signal); }
            : null,
          onDelivered: function () {
            var rel = relationOfSession(sessionId) || relationOfSession(st.newId);
            if (rel) retargetRelation(rel.rowId, st.newId);
          }
        }).then(function () {
          if (parentSel && ctxLike.modelSel && typeof ctxLike.modelSel.apply === "function") {
            try { Promise.resolve(ctxLike.modelSel.apply(st.newId, parentSel)); log("info", "wm-assistant", "子会话已继承模型/挡位", { child: st.newId, sel: parentSel }); } catch (eApply) { /* 继承失败按默认 */ }
          }
          log("info", "wm-assistant", "编辑完成", { child: st.newId, seedEvents: st.seedEvents, attach: st.attach });
          wmToast(TEXT.wmEditDone, "info");
          return true;
        });
      }, function (err) {
        var msg = String(err && err.message ? err.message : err);
        wmToast(msg, "error");
        log("warn", "wm-assistant", "编辑回复失败", { sessionId: sessionId, err: msg });
        throw err;
      });
    }
    /** 助手消息操作区的「编辑这条回复」按钮。 */
    function AssistantEditAction(props) {
      var sessionId = props && props.sessionId;
      var messageId = props && props.messageId;
      var busyState = React.useState(false);
      var busy = busyState[0];
      var setBusy = busyState[1];
      // 自诊断：前几次渲染把"槽位到底给了什么"写进宿主日志（排障用，限次）。
      try {
        wmAssistDiag.renders += 1;
        if (wmAssistDiag.logged < 5 && messageId && !wmAssistDiag["m:" + messageId]) {
          wmAssistDiag.logged += 1;
          wmAssistDiag["m:" + messageId] = true;
          log("info", "wm-assistant", "编辑回复按钮渲染", {
            sessionId: sessionId || null,
            messageId: messageId || null,
            propsKeys: Object.keys(props || {}).slice(0, 24),
            hasCtxSessions: !!(props && props.ctxSessions && typeof props.ctxSessions.fork === "function")
          });
        }
      } catch (eDiag) { /* 诊断失败不影响渲染 */ }
      // 只依赖槽位必给的 sessionId + messageId：解析一律交给宿主（见 resolveAssistantTarget）。
      ...
      function onClick(e) {
        ...
        setBusy(true);
        var dlg = null;
        resolveAssistantTarget(sessionId, messageId, null).then(function (resolved) {
          dlg = openAssistantEditDialog({
            text: typeof resolved.oldText === "string" ? resolved.oldText : "",
            reasoningText: typeof resolved.reasoningText === "string" ? resolved.reasoningText : "",
            onConfirm: function (newText, blockKind) {
              return runAssistantEdit(
                {
                  ctxSessions: props.ctxSessions,
                  openSession: props.openSession,
                ...
                sessionId, resolved, newText, dlg, blockKind
              ).then(function (done) { setBusy(false); return done; }, function (err) {
                setBusy(false);
                if (dlg && dlg.setStatus) dlg.setStatus(String(err && err.message ? err.message : err));
                return false;
              });
            }
          });
          setBusy(false);
          log("info", "wm-assistant", "编辑框已打开", { sessionId: sessionId, messageId: messageId, resolvedBy: resolved.resolvedBy, oldLen: (resolved.oldText || "").length });
        }, function (err) {
          setBusy(false);
          var msg = String(err && err.message ? err.message : err);
          wmToast(msg, "error");
          log("warn", "wm-assistant", "解析目标回复失败", { sessionId: sessionId, messageId: messageId, code: err && err.code, err: msg });
        });
      }
      return React.createElement("button", {
        type: "button",
        title: TEXT.wmEditAssistant,
        "aria-label": TEXT.wmEditAssistant,
        "data-dsh-message-recall": "assistant-edit",
        disabled: busy,
        style: {
          border: "none",
          background: "transparent",
          ...
          alignItems: "center",
          justifyContent: "center",
          color: "var(--dsw-alias-label-secondary, #cfcfcf)",
          fontSize: "15px",
          lineHeight: "19px",
          fontFamily: "inherit"
        },
        onMouseEnter: function (e) { e.currentTarget.style.background = "var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,0.12))"; },
        onMouseLeave: function (e) { e.currentTarget.style.background = "transparent"; },
        onClick: onClick
      }, "✎");
    }
    ...
    /**
     * 助手消息操作区的「用原提问重新生成」按钮（↻）。
     *
     * 借鉴社区 dsh-message-edit 的"回合原子性"：分支点取**目标回合之前**（turn/start - 1），
     * 该回合整段不进新会话；原用户输入交给**底座已有的 resume-send 管道**重发
     * （与"编辑我的消息"同一条已验证路径：写 RESUME_PREFIX 记录 → 打开子会话 → claimResume 自动投递）。
     */
    function AssistantRegenerateAction(props) {
      var sessionId = props && props.sessionId;
      var messageId = props && props.messageId;
      var busyState = React.useState(false);
      var busy = busyState[0];
      ...
      function onClick(e) {
        ...
        if (preMembers.indexOf(expectedId) < 0) preMembers.push(expectedId);
        var parentSel = null;
        try { if (props.modelSel && typeof props.modelSel.capture === "function") parentSel = props.modelSel.capture(sessionId); } catch (eSel) { parentSel = null; }

        registerRelation(rowId, expectedId, preMembers).then(function (preRegistered) {
          return fetch("/bubble/regenerate", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ parentSessionId: sessionId, childSessionId: expectedId, messageId: messageId })
          }).then(function (r) { return r.json().then(function (d) { return d || {}; }); }).then(function (d) {
            if (!d.ok) {
              if (preRegistered) {
                var keep = [];
                for (var i = 0; i < preMembers.length; i++) if (preMembers[i] !== expectedId) keep.push(preMembers[i]);
              ...
              var code = d.error || "unknown";
              var msg = code === "no-user-input" ? TEXT.wmRegenerateNoInput : (d.message || TEXT.wmEditOpenFail);
              throw new Error(msg);
            }
            var newId = d.childSessionId || expectedId;
            // 关键：把原提问写进 resume 记录 —— 打开子会话后由底座管道自动重发（这条路径已在"编辑我的消息"验证过）
            try {
              localStorage.setItem(RESUME_PREFIX + newId, JSON.stringify({
                draftText: typeof d.userText === "string" ? d.userText : "",
                t: Date.now(),
                imageIds: [],
                sel: parentSel,
                stagedDraft: null
              }));
            } catch (eLs) { log("warn", "wm-assistant", "重生成：resume 记录写入失败（需手动发送）", { child: newId }); }
            return { newId: newId, expectedId: expectedId, rowId: rowId, preRegistered: preRegistered, turn: d.turn, attach: d.attach, partial: d.partial === true };
          });
        }).then(function (st) {
          var chain = Promise.resolve(st.preRegistered === true);
          if (st.newId !== st.expectedId) chain = Promise.resolve(reconcileForkChild(st.rowId, st.expectedId, st.newId, sessionId));
          ...
          try { claimResume(st.newId); } catch (eClaim) { /* 订阅兜底仍会认领 */ }
          log("info", "wm-assistant", "重生成分支已建好，开始打开", { parent: sessionId, child: st.newId, turn: st.turn, attach: st.attach, partial: st.partial });
          return controlledSwitch({
            opKey: "wmregen:" + sessionId + ">" + st.newId,
            sourceId: sessionId,
            targetId: st.newId,
            ctxSessions: props.ctxSessions,
            openSession: function (id) { try { props.openSession(id); } catch (eO) { /* ignore */ } },
            archiveSession: props.archiveSession,
            restoreSession: props.restoreSession,
            prepareSession: (props.ctxSessions && typeof props.ctxSessions.prepare === "function")
              ? function (id, signal) { return props.ctxSessions.prepare(id, signal); }
            ...
            setBusy(false);
            wmToast(st.partial ? TEXT.wmEditDone + "（未归入工作区，重启后自动归位）" : TEXT.wmRegenerateDone, st.partial ? "warn" : "info");
            return true;
          });
        }, function (err) {
          setBusy(false);
          var msg = String(err && err.message ? err.message : err);
          wmToast(msg, "error");
          log("warn", "wm-assistant", "重生成失败", { sessionId: sessionId, messageId: messageId, err: msg });
          return false;
        });
      }
      ...
      return React.createElement("button", {
        type: "button",
        title: TEXT.wmRegenerate,
        "aria-label": TEXT.wmRegenerate,
        "data-dsh-message-recall": "assistant-regenerate",
        disabled: busy,
        style: {
          border: "none",
          background: "transparent",
...
React.createElement(
    ...
    function apply(ctx) {
      ctx.effect(function () {
        // 隔离：本插件任何异常都不得影响宿主其余 UI（曾因一处异常导致整块贡献被撤下）。
        try {
          return wmAuxApply(ctx);
        } catch (errWm) {
          log("error", "lifecycle", "client half 异常（已隔离：本插件贡献撤下，宿主官方渲染接管）", {
            err: String(errWm && errWm.message ? errWm.message : errWm),
            stack: String(errWm && errWm.stack ? errWm.stack : "").slice(0, 1200)
          });
          return void 0;
        }
    ...
    function wmAuxApply(ctx) {
        ...
        function gatedSlot(name, registerFn) {
          ...
          var run = function () {
            ...
            try { current = registerFn(); } catch (e) {
              gateErrors.push({ slot: name, err: String(e && e.message ? e.message : e) });
        ...
        // WM 分叉：助手消息操作区的「编辑这条回复」按钮（同一槽位，order 20 排在翻页器之后）。
        // 机制与「编辑我的消息」一致：宿主办界 → 官方 fork 开分支 → 冷态子会话里改写回复文本
        // → 受控切换打开 → 归档父会话（旧版本）；不写 resume 记录，因此不会自动重发任何内容。
        gatedSlot("conversation.chat.assistant-actions", function () {
          return ctx.slots.register({
            name: "conversation.chat.assistant-actions",
            id: "dsh-message-recall-wm-assistant-edit",
            order: 20,
            inject: function () {
              return {
                openSession: function (id) { ctx.sessions.open(id); },
                ...
                restoreSession: function (id) {
                  return fetch("/bubble/unarchive", {
                    ...
                    body: JSON.stringify({ sessionId: id }),
                    keepalive: true
                  }).then(function (r) { return r.json(); }).then(function (d) { return !!(d && d.ok); }).catch(function () { return false; });
                }
              };
            }
          }, AssistantEditAction);
        });
        // WM 分叉：助手消息操作区的「用原提问重新生成」按钮（同一槽位，order 25）。
        gatedSlot("conversation.chat.assistant-actions", function () {
          return ctx.slots.register({
            name: "conversation.chat.assistant-actions",
            id: "dsh-message-recall-wm-regenerate",
            order: 25,
            inject: function () {
              return {
                openSession: function (id) { ctx.sessions.open(id); },
                ...
                archiveSession: function (id) {
                  try {
                    return Promise.resolve(ctx.workspaces.archiveSession(id)).then(function (ok) { return ok !== false; }).catch(function () { return false; });
                  } catch (e) { return Promise.resolve(false); }
                },
                restoreSession: function (id) {
                  return fetch("/bubble/unarchive", {
        ...
        // WM 就绪自检：一眼看清门控状态与各槽位注册结果（排障用）。
        log("info", "lifecycle", "wm 就绪自检", { enabled: getEnabledSnapshot(), runners: gateRunners.length, gateErrors: gateErrors, settingsCard: typeof d4 === "function" });
        return function () {
    ...
    return { name: "dsh-message-recall-wm", inject: ["slots", "sessions", "workspaces", "conversation", "locale", "modelDirectories"], apply: apply };
  
    });

    // ======== manager：会话/工作区管理（含工作区真迁移） ========
    halfFactories.push(function () {
    ...
    const css = [
      ...
      "[data-sm-theme=dark] .sm-panelDialog,[data-sm-theme=dark] .sm-confirmDialog.sm-nativeDialogLayer .sm-nativeDialog,[data-sm-theme=dark] .sm-migrateDialog.sm-nativeDialogLayer .sm-nativeDialog{--dsw-alias-surface-l1:#1f1f23;--dsw-alias-label-primary:#ececec;--dsw-alias-label-secondary:#b3b3b3;--dsw-alias-label-tertiary:#999;--dsw-alias-border-l1:#555;--dsw-alias-border-l2:#3a3a40;--dsw-alias-fill-l2:#2a2a30;--dsw-alias-interactive-bg-hover:rgba(255,255,255,.06);--dsw-alias-state-error-primary:#f47171;--dsw-alias-state-warning-primary:#f0a040;--dsw-alias-accent-primary:#5d8aff;--dsw-alias-accent-primary-bg:rgba(93,138,255,.18);background:#1f1f23!important;background-color:#1f1f23!important;color:#ececec!important;border-color:#3a3a40!important}",
    ...
    /* === wm fork: workspace folder migration ===
     *
     * Adds "迁移到新文件夹…" to the built-in workspace ⋯ menu. The menu entry
     * itself is injected by the host half's guarded overlay patch
     * (lib/workspace-menu-patch.js); this half only supplies the two hooks the
     * patch calls, so an unpatched menu behaves exactly like upstream.
     *
     * The dialog is a plain React component rendered through the bundle's own
     * imperative portal helper, and the request reuses callApi().
     */
    const wmZh = () => {
      try {
        if ((navigator.language || "").toLowerCase().startsWith("zh")) return true;
        return (document.documentElement.getAttribute("lang") || "").toLowerCase().startsWith("zh");
      } catch (_) { return true; }
    };
    const wmT = (zhText, enText) => (wmZh() ? zhText : enText);

    function WmMigrateDialog(props) {
      const { row, onClose } = props;
      const [value, setValue] = useState(typeof row?.cwd === "string" ? row.cwd : "");
      ...
      useEffect(() => {
        ...
        if (node) { try { node.focus(); node.select(); } catch (_) { /* ignore */ } }
      }, []);
      useEffect(() => {
        const onKey = (event) => { if (event.key === "Escape" && !busy && done === null) onClose(); };
        window.addEventListener("keydown", onKey, true);
      ...
      const submit = async () => {
        ...
        setError(null);
        try {
          const payload = await callApi("/workspace-migrate", {
            workspaceId: row?.workspaceId,
            from: row?.cwd,
            to: target
          });
          if (payload?.ok === false) throw new Error(payload.error || "迁移失败");
          const result = payload?.result ?? {};
          setDone({
            newPath: result.newPath,
            folder: result.folder,
            moved: Array.isArray(result.movedSessions) ? result.movedSessions.length : 0,
            failed: Array.isArray(result.failed) ? result.failed : []
          });
          window.setTimeout(() => { try { window.location.reload(); } catch (_) { /* ignore */ } }, 1500);
        } catch (err) {
      ...
      return h("div", { className: "sm-nativeDialogLayer sm-confirmDialog" },
        h("div", { className: "sm-nativeDialog", role: "dialog", "aria-modal": "true" },
          h("div", { className: "sm-nativeDialogHeader" },
            h("h2", { className: "sm-nativeDialogTitle" }, wmT("迁移工作区", "Migrate workspace"))),
          h("div", { className: "sm-nativeDialogBody" },
            h("div", { className: "sm-nativeDialogDescription" },
              wmT(
                "把该工作区迁移到新的文件夹路径。填一个还不存在的路径：连文件夹一起搬过去；填一个已经搬好的路径：只修复 DSH 的记录与日志归属。",
                "Migrate this workspace to a new folder path. A path that does not exist yet is moved there; a path you already moved is simply re-linked."
              )),
            h("div", { className: "sm-nativeDialogDescription", style: { marginTop: "6px" } },
              wmT("当前路径：", "Current path: ") + String(row?.cwd ?? "")),
            h("input", {
              ref: inputRef,
              value,
            ...
            error !== null
              ? h("div", { className: "sm-nativeDialogDescription", style: { marginTop: "8px", color: "var(--dsw-alias-state-error-primary,#d92d20)" } }, error)
              : null,
            done !== null
              ? h("div", { className: "sm-nativeDialogDescription", style: { marginTop: "8px" } },
                  wmT("迁移完成：", "Migrated: ") + String(done.newPath ?? "") +
                  wmT(`（文件夹 ${done.folder}，搬移会话 ${done.moved} 条）`, ` (folder ${done.folder}, sessions ${done.moved})`) +
                  (done.failed.length > 0 ? wmT(`，失败 ${done.failed.length} 条`, `, failed ${done.failed.length}`) : ""))
              : null),
          h("div", { className: "sm-nativeDialogFooter" },
            h("button", {
              type: "button",
            ...
            h("button", {
              ...
              disabled: busy || done !== null,
              onClick: () => void submit()
            }, busy ? wmT("迁移中…", "Migrating…") : wmT("开始迁移", "Migrate")))));
    }

    const openWorkspaceMigrateDialog = (row) => {
      try {
        const host = document.createElement("div");
        document.body.appendChild(host);
        const close = () => {
          try { host.__smRoot?.unmount?.(); } catch (_) { /* ignore */ }
          try { host.remove(); } catch (_) { /* ignore */ }
        };
        window.__smRenderInto(h(WmMigrateDialog, { row, onClose: close }), host);
      } catch (error) {
        console.warn("[session-manager-wm] 打开工作区迁移对话框失败", error);
      }
    };
    ...
    globalThis.__DSH_WM__ = {
      ...
      handleWorkspaceMenu: (id, row) => {
        ...
        openWorkspaceMigrateDialog(row);
        return true;
      }
    };
    ...
    // ======== delete：消息删除（按条删指令/回复、按步骤删思考与工具调用） ========
    halfFactories.push(function () {
    ...
    for (var i = 0; i < halfFactories.length; i++) {
      try {
        ...
        try { console.error("[dsh-wm-toolkit] 半边工厂执行失败", i, eHalf); } catch (eLog) { /* ignore */ }
      }
    }
    var inject = [];
      ...
      apply: function (ctx) {
        ...
        faces.forEach(function (face) {
          try { face.apply(ctx); } catch (eApply) {
            ...
            try { console.error("[dsh-wm-toolkit] 半边应用失败（已隔离）", face.name, eApply); } catch (eLog2) { /* ignore */ }
          }
        });
        if (faces.length > 0 && failures.length === faces.length) {
          throw new Error("dsh-wm-toolkit: 浏览器半边全部未能加载 — " + failures.join(" | "));
        }
      }
    };
  }
