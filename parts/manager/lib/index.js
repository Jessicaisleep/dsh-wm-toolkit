...
${rest}`, "utf8");
    ...
    const moveSession = async (sessionId, targetWorkspaceId) => {
      ...
      if (storedHeader.cwd !== void 0) {
        ...
        if (currentCanonical === targetPath) {
          /* wm fork: the path already matches, but grouping is decided by the
             workspace's ACCOUNTING, not by the path. An ungrouped session whose
             cwd happens to sit in the target folder therefore used to no-op here
             forever ("未分组对话 → 移动到工作区 不生效"). Attach it instead: the
             canonical-cwd check inside attachSession() passes for exactly this
             case, so this only repairs the missing record entry. */
          const alreadyAccounted = Array.isArray(target.record?.sessionIds)
            && target.record.sessionIds.includes(sessionId);
          if (alreadyAccounted) {
          ...
          try {
            ...
            /* attachSession() validates the cwd through the registry cache, which
               may still hold the pre-migration spelling. Repair the index first,
               then account the id through the record so the command still works. */
            registry.sessionPaths?.set?.(sessionId, targetPath);
            registry.invalidSessionPaths?.delete?.(sessionId);
            const previousHeader = registry.headers?.get?.(sessionId);
            if (previousHeader !== undefined) registry.headers.set(sessionId, { ...previousHeader, cwd: targetPath });
            await target.mutate((record) => {
              const ids = Array.isArray(record.sessionIds) ? record.sessionIds : [];
              return {
                ...record,
                sessionIds: ids.includes(sessionId) ? ids : [sessionId, ...ids],
                updatedAt: new Date().toISOString(),
              };
            });
            ctx.logger.warn(`session-manager(wm): attachSession failed for "${sessionId}", repaired accounting directly: ${String(error)}`);
          }
          ctx.logger.info(`session-manager(wm): attached already-located session "${sessionId}" to workspace "${target.id}"`);
          return { ok: true, sessionId, moved: false, attached: true, message: "会话路径已在该工作区内，已补上归属（记账）" };
        }
      }
    ...
    /* === workspace folder migration (wm fork addition) ===
     *
     * Migrate a whole workspace to a new folder path. DSH binds a session to a
     * workspace through `realpath(header.cwd) === workspace.path`, and binds a
     * log's physical location to that same cwd, so a folder move has to be
     * repaired in three places at once: the workspace record's path, each
     * session's stored cwd, and each session's artifact directory.
     *
     * This reuses the plugin's own moveSession() so the artifact relocation,
     * frame-0-safe header rewrite, live-writer rebinding and projection-cache
     * refresh all behave exactly like a cross-workspace move:
     *   1. move the folder (when the target does not exist yet),
     *   2. repoint the workspace record's path first, so moveSession() resolves
     *      the new path as its target and only re-homes the artifacts,
     *   3. walk every session that belongs to the workspace (accounted ids plus
     *      any session still carrying the old cwd, which is how unaccounted
     *      "orphan" sessions show up) and move each one.
     */
    const migrateWorkspace = async (body) => {
      const registry = ctx.workspaceRegistry;
      const persistence = ctx.get("sessionPersistence");
      const dshHomePath = ctx.get("dshHomePath");
      if (registry === void 0) throw new Error("workspaceRegistry 服务不可用");
      const wantedId = typeof body?.workspaceId === "string" && body.workspaceId.trim() !== ""
        ? body.workspaceId.trim()
        : null;
      const wantedPath = typeof body?.from === "string" && body.from.trim() !== ""
        ? body.from.trim()
        : null;
      const to = typeof body?.to === "string" && body.to.trim() !== "" ? body.to.trim() : null;
      if (to === null) throw new Error("新文件夹路径 to 必填");

      const entity = registry.list().find((candidate) =>
        (wantedId !== null && candidate.id === wantedId) ||
        (wantedPath !== null && String(candidate.path).toLowerCase() === wantedPath.toLowerCase()));
      if (entity === void 0) throw new Error("找不到目标工作区");

      const oldPath = entity.path;
      /* Capture the accounted ids BEFORE repointing the record: entity.sessionIds
         is filtered by `realpath(cwd) === record.path`, so it empties out the
      ...
      try { newPath = await realpath(rawTarget); } catch { /* target does not exist yet: keep the raw path */ }
      if (newPath.toLowerCase() === String(oldPath).toLowerCase()) {
        return { ok: true, moved: false, message: "新路径与当前路径相同", oldPath, newPath, folder: "untouched", movedSessions: [], failed: [] };
      }

      let targetIsDir = false;
      try { targetIsDir = (await stat(newPath)).isDirectory(); } catch { targetIsDir = false; }
      let folder = "untouched";
      if (!targetIsDir) {
        let oldIsDir = false;
        try { oldIsDir = (await stat(oldPath)).isDirectory(); } catch { oldIsDir = false; }
        if (!oldIsDir) throw new Error(`旧路径与新路径都不存在：${oldPath} / ${newPath}`);
        /* Create the parent ONLY when it is genuinely missing. A drive root
           (D:\ or /) already exists and must never be mkdir'ed — doing so was
           the source of `EPERM: operation not permitted, mkdir 'D:\'`. */
        const parent = dirname(newPath);
        if (parent !== newPath) {
          let parentIsDir = false;
          try { parentIsDir = (await stat(parent)).isDirectory(); } catch { parentIsDir = false; }
          if (!parentIsDir) await mkdir(parent, { recursive: true });
        }
        try {
      ...
      /* Use the CANONICAL spelling of the folder from here on: membership is
         decided by `realpath(session cwd) === record.path`, so a typed path whose
         casing differs from the on-disk name (d:\x vs D:\x) would leave every
         session unmatched and ungrouped. */
      try { newPath = await realpath(newPath); } catch { /* keep the raw spelling */ }

      /* The workspace-migration operation is owned by the file-level module that
         was validated in practice (lib/wm-relocate.js): one relocation pass over
         every session whose stored cwd is the old path — frame-0 header rewrite,
         transactional directory move, projection-cache sync — then the workspace
         record, then the index refresh. Session-level move/delete keep upstream's
         own implementation and are not involved here. */
      ...
      if (typeof sessionsRoot !== "string" || typeof projcacheDir !== "string") {
        throw new Error("dshHomePath 服务不可用，无法定位会话目录");
      }

      /* Sessions DSH currently holds for the old path: flush their pending
         events, and require a rebindable live writer — otherwise refuse the
         migration instead of writing under a log the runtime still owns. */
      const liveSessions = [];
      for (const session of ctx.sessions.list()) {
        const cwd = session?.header?.cwd;
        if (typeof cwd !== "string" || cwd.toLowerCase() !== String(oldPath).toLowerCase()) continue;
        if (liveWriterOf(persistence, String(session.id)) === undefined) {
          throw new Error(
            `会话 ${String(session.id)} 正在被 DSH 打开，且当前运行时不支持安全重绑定；请先关闭它，或在重启 DSH 后立即执行迁移。`,
          );
        }
      ...
      const relocation = relocateSessions({ sessionsRoot, projcacheDir, fromCwd: oldPath, toCwd: newPath });
      if (relocation.failed.length > 0) {
        throw new Error(`部分会话迁移失败（已完成的会话保持自洽）：${JSON.stringify(relocation.failed)}`);
      }

      /* Point every live session at the new cwd: rewrite the in-memory header and
         retarget the writer it already holds, so the open conversation keeps
         working and later writes land in the relocated log. */
      const liveRebound = [];
      for (const session of liveSessions) {
        try {
          ...
          ctx.logger.warn(`session-manager(wm): 活动会话重绑定失败 ${String(session?.id)}：${String(error)}`);
        }
      }

      /* Make the registry SEE the new cwd for every relocated session BEFORE the
         record is repointed. `entity.mutate()` prunes every id failing the
         "canonical cwd === record.path" membership check, and the persistence
         listing can serve cached headers — so the reliable move is the same
         bookkeeping upstream's moveSession() performs: patch the registry's own
         path/header maps directly. */
      for (const id of relocation.moved) {
        try {
          registry.sessionPaths?.set?.(id, newPath);
          registry.invalidSessionPaths?.delete?.(id);
          const previous = registry.headers?.get?.(id);
          if (previous !== undefined) registry.headers.set(id, { ...previous, cwd: newPath });
        } catch { /* per-session best-effort */ }
        try { persistence?.coordinator?.preparations?.invalidate?.(id); } catch { /* best-effort */ }
      }
      try {
      ...
      /* Repoint the record, and account every relocated session explicitly:
         sessions that were already orphaned (ungrouped but physically inside
         this folder) are repaired by the same pass. */
      await entity.mutate((record) => {
        const existing = Array.isArray(record.sessionIds) ? record.sessionIds : [];
        const missing = relocation.moved.filter((id) => !existing.includes(id));
        return {
          ...record,
          path: newPath,
          sessionIds: missing.length === 0 ? existing : [...existing, ...missing],
          updatedAt: new Date().toISOString(),
        };
      });

      /* Rebuild entities and re-read the index so the GUI regroups immediately,
         then report whether membership actually resolves: `record` counts the
         accounted ids, `visible` the ones that pass the path check. */
      try { registry.rebuildEntities?.(); } catch { /* best-effort */ }
      const membership = { sessionIds: 0, visible: 0 };
      try {
        const fresh = registry.list().find((candidate) => candidate.id === entity.id);
        membership.sessionIds = Array.isArray(fresh?.record?.sessionIds) ? fresh.record.sessionIds.length : 0;
        membership.visible = Array.isArray(fresh?.sessionIds) ? fresh.sessionIds.length : 0;
        ctx.logger.info(
          `session-manager(wm): membership after migration = record ${membership.sessionIds} / visible ${membership.visible} (path=${newPath})`,
        );
        for (const id of relocation.moved.slice(0, 20)) {
          let indexed;
          try { indexed = registry.sessionPaths?.get?.(id); } catch { indexed = undefined; }
          ctx.logger.info(`session-manager(wm): index path for "${id}" = ${String(indexed)}`);
        }
      } catch { /* best-effort */ }

      /* Blank placeholders inside the migrated workspace are removed so the
         sidebar never shows a stray empty conversation at the new path. */
      const cleanedPlaceholders = [];
      for (const entry of listSessions(sessionsRoot)) {
        try {
          const cache = join(projcacheDir, entry.sessionId + ".json");
          if (!existsSync(cache)) continue;
          const parsed = JSON.parse(readFileSync(cache, "utf8"));
          const blank = parsed?.record?.rows?.sessionListMetadata?.val?.blank === true;
          const cwd = parsed?.record?.identity?.cwd;
          if (blank !== true || typeof cwd !== "string") continue;
          if (cwd.toLowerCase() !== String(newPath).toLowerCase()) continue;
          const removed = deleteSessionFiles({ sessionsRoot, projcacheDir, sessionId: entry.sessionId });
          if (removed.removed.length > 0) cleanedPlaceholders.push(entry.sessionId);
        } catch { /* per-session best-effort */ }
      }
      ...
      ctx.logger.info(
        `session-manager(wm): migrated workspace "${entity.id}" ${oldPath} -> ${newPath} (folder=${folder}, moved=${relocation.moved.length}, liveRebound=${liveRebound.length}, blanks=${cleanedPlaceholders.length})`,
      );
      return {
        ok: true,
        moved: true,
        ...
        newPath,
        folder,
        movedSessions: relocation.moved.map((id) => ({ id, moved: true })),
        liveRebound,
        cleanedPlaceholders,
        failed: [],
      };
...
ctx.logger.warn(`session-manager: api error: ${String(error)}; type=${error?.constructor?.name}; json=${safeJson(error)}`);
          ...
          /* wm fork: migrate a whole workspace folder (repairs the workspace
             record, every member session's stored cwd and its artifact dir). */
          if (req.method === "POST" && path === "/workspace-migrate") {
            try {
              const result = await migrateWorkspace(body);
              return send(res, 200, { ok: true, result });
            } catch (error) {
              ctx.logger.warn(`session-manager(wm): workspace-migrate failed: ${String(error)}`);
              return send(res, 500, { ok: false, error: String(error instanceof Error ? error.message : error) });
            }
          }
