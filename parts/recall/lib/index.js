/**
 * dsh-message-recall-wm — node half。
 *
 * host 路由：
...
import { resolveAssistantBoundary, editAssistantText, branchAssistantEdit, regenerateBranch } from './wm-assistant-edit.js';
...
function resolveLogFile() {
  ...
  logFile = join(home, 'dsh-message-recall-wm.log');
  return logFile;
}
...
function writeLog(level, tag, message, data) {
  ...
  if (process.env.DSH_MESSAGE_RECALL_DEBUG === '1') {
    if (level === 'error') console.error('[dsh-message-recall-wm]', tag, message, data ?? '');
    else if (level === 'warn') console.warn('[dsh-message-recall-wm]', tag, message, data ?? '');
    else console.info('[dsh-message-recall-wm]', tag, message, data ?? '');
  }
...
export const name = 'dsh-message-recall-wm'
export const inject = ['webServer', 'sessions', 'settings', 'storageDomain', 'workspaceRegistry', 'agentPresets', 'agents', 'sessionPersistence']
 ...
 * 更新通道。
 *
 * 本仓库是"组合版"：两个半边分别来自 dsh-message-recall / dsh-session-manager 的 MIT 分叉，
 * 由 dsh-wm-toolkit 组合成**一个插件**发布。npm 上不存在 'dsh-message-recall-wm' 这个包名，
 * 上游包名也绝不能被本插件的"更新"按钮升级（会把分叉覆盖掉），所以这里显式声明：无更新通道，
 * 检查更新只回报当前版本与"主包名"，更新动作直接拒绝并说明正确的升级方式。
 */
const UPDATE_CHANNEL = 'none';
const OWN_PACKAGE_NAME = 'dsh-wm-toolkit';
/**
 * 读取本插件自身的版本号。
 *
...
async function fetchLatestVersion() {
  try {
    const resp = await fetch('https://registry.npmjs.org/dsh-message-recall-wm/latest', {
      headers: { 'user-agent': 'dsh-message-recall-wm-update-check', accept: 'application/json' }
    });
...
export function apply(ctx) {
  ...
  /**
   * 自愈：把"已经不存在的会话"从版本家族里剔除。
   *
   * 为什么需要：即使某个成员被外部清掉（或被历史版本的行为误伤），派生的关系记录仍会留着
   * 它的 id，翻页器就会出现点不动的版本（`sessions.select: unknown session`）。这里用官方的
   * `workspaceRegistry.sessionKnown()` 判存在性（归档会话也算存在），只剔除真正没了的成员。
   */
  async function pruneDeadMembers(row) {
    try {
      const registry = ctx.get('workspaceRegistry');
      if (!registry || typeof registry.sessionKnown !== 'function') return row;
      const known = [];
      for (const id of row.memberSessionIds) {
        if (id === row.targetSessionId) { known.push(id); continue; }
        let alive = true;
        try { alive = await registry.sessionKnown(id); } catch { alive = true; } // 判不了就当活着，绝不误删
        if (alive) known.push(id);
        else writeLog('warn', 'relations', '剔除已不存在的版本成员', { rowId: row.rowId, sessionId: id });
      }
      if (known.length === 0) known.push(row.targetSessionId);
      if (known.length === row.memberSessionIds.length) return row;
      const target = known.includes(row.targetSessionId) ? row.targetSessionId : known[known.length - 1];
      return await writeRelation({ ...row, targetSessionId: target, memberSessionIds: known, updatedAt: Date.now() });
    } catch (err) {
      writeLog('warn', 'relations', '版本家族自愈失败（保持原样）', { rowId: row.rowId, err: String(err?.message ?? err) });
      return row;
    }
  }
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const rows = readAllRelations();
        const next = [];
        for (const row of rows) next.push(await pruneDeadMembers(row));
        sendJson(res, 200, { ok: true, relations: next });
      } catch (err) {
        sendJson(res, 500, { ok: false, error: 'internal' });
      }
    }
  ...
  // 语义：与"编辑我的消息"同一套机制（归档旧会话 + 开新分支），只多一步：
  //   fork 边界取目标回合**自己的** turn/end（子会话保留该回合），
  //   然后在冷态子会话里把那条回复的文本改掉，再打开子会话。
  // 该回合之后的内容留在旧会话里（与编辑自己的消息一致）。
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/bubble/recall-assistant',
    handler: async (req, res) => {
      try {
        if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method-not-allowed' }); return; }
        const body = await readJsonBody(req);
        const { sessionId, targetSeq, turn, messageId, blockKind } = body;
        if (!guard(req, res, true, sessionId)
          || (typeof targetSeq !== 'number' && typeof turn !== 'number' && typeof messageId !== 'string')) {
          sendJson(res, 400, { ok: false, error: 'invalid-request' });
          return;
        }
        writeLog('info', 'assistant-edit', '收到助手回复编辑边界请求', { sessionId, targetSeq, turn, messageId: messageId ?? null });
        const result = resolveAssistantBoundary(ctx, sessionId, {
          targetSeq: typeof targetSeq === 'number' && Number.isFinite(targetSeq) ? targetSeq : undefined,
          turn: typeof turn === 'number' && Number.isFinite(turn) ? turn : undefined,
          messageId: typeof messageId === 'string' && messageId.length > 0 ? messageId : undefined,
        });
        if (result.code) {
          writeLog('warn', 'assistant-edit', '编辑被拒绝: ' + result.code, { sessionId, message: result.message });
          sendJson(res, result.status, { ok: false, error: result.code, message: result.message });
          return;
        }
        sendJson(res, 200, {
          ...
          reasoningIndex: result.reasoningIndex ?? null,
  ...
  // ---- 主路径：建分支时就用改好的种子（从根上没有"活动会话内存副本是旧文本"的问题）----
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/bubble/assistant-branch',
    handler: async (req, res) => {
      try {
        if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method-not-allowed' }); return; }
        const body = await readJsonBody(req, 512 * 1024);
        const { parentSessionId, childSessionId, targetSeq, turn, messageId, newText, replaceAllTextInMessage, blockKind, blockIndex } = body;
        if (!guard(req, res, true, parentSessionId) || typeof newText !== 'string' || typeof childSessionId !== 'string') {
          sendJson(res, 400, { ok: false, error: 'invalid-request' });
          return;
        }
        writeLog('info', 'assistant-branch', '收到建分支请求', {
          parentSessionId, childSessionId, targetSeq, turn, messageId: messageId ?? null, chars: newText.length,
        });
        const result = await branchAssistantEdit(ctx, writeLog, {
          parentSessionId, childSessionId,
          targetSeq: typeof targetSeq === 'number' && Number.isFinite(targetSeq) ? targetSeq : undefined,
          turn: typeof turn === 'number' && Number.isFinite(turn) ? turn : undefined,
          messageId: typeof messageId === 'string' && messageId.length > 0 ? messageId : undefined,
          newText,
          replaceAllTextInMessage: replaceAllTextInMessage === true,
          blockKind: blockKind === 'reasoning' ? 'reasoning' : (blockKind === 'text' ? 'text' : undefined),
          blockIndex: Number.isSafeInteger(blockIndex) ? blockIndex : undefined,
        });
        sendJson(res, 200, { ok: true, ...result });
      } catch (err) {
        const code = err?.code ?? 'internal';
        writeLog('error', 'assistant-branch', '/bubble/assistant-branch 失败', { code, message: String(err?.message ?? err) });
        sendJson(res, 500, { ok: false, error: code, message: String(err?.message ?? err) });
      }
    }
  }));
  // ---- 重新生成 / 重试任意回合（借鉴社区 dsh-message-edit 的"回合原子性"）----
  // 分支点取目标回合**之前**（turn/start - 1），该回合整段不进新会话；
  // 原用户输入由客户端写进 resume 记录，打开子会话后由底座已验证的 resume-send 管道自动重发。
  // 不指定 turn/targetSeq/messageId 时 = 重新生成最后一条带文本的回复所属回合。
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/bubble/regenerate',
    handler: async (req, res) => {
      try {
        if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method-not-allowed' }); return; }
        const body = await readJsonBody(req, 64 * 1024);
        const { parentSessionId, childSessionId, turn, targetSeq, messageId } = body;
        if (!guard(req, res, true, parentSessionId) || typeof childSessionId !== 'string') {
          sendJson(res, 400, { ok: false, error: 'invalid-request' });
          return;
        }
        writeLog('info', 'assistant-branch', '收到重生成请求', {
          parentSessionId, childSessionId, turn: turn ?? null, targetSeq: targetSeq ?? null, messageId: messageId ?? null,
        });
        const result = await regenerateBranch(ctx, writeLog, {
          parentSessionId, childSessionId,
          turn: typeof turn === 'number' && Number.isFinite(turn) ? turn : undefined,
          targetSeq: typeof targetSeq === 'number' && Number.isFinite(targetSeq) ? targetSeq : undefined,
          messageId: typeof messageId === 'string' && messageId.length > 0 ? messageId : undefined,
        ...
        const code = err?.code ?? 'internal';
        writeLog('error', 'assistant-branch', '/bubble/regenerate 失败', { code, message: String(err?.message ?? err) });
        sendJson(res, 500, { ok: false, error: code, message: String(err?.message ?? err) });
      }
    }
  }));
  // 修复工具：改写既有会话里的回复文本（不动内存，界面刷新后可见）
  disposers.push(ctx.webServer.register({
    kind: 'exact',
    path: '/bubble/edit-assistant',
    handler: async (req, res) => {
      try {
        if (req.method !== 'POST') { sendJson(res, 405, { ok: false, error: 'method-not-allowed' }); return; }
        const body = await readJsonBody(req, 512 * 1024);
        const { sessionId, targetSeq, turn, oldText, newText, replaceAllTextInMessage, messageId } = body;
        if (!guard(req, res, true, sessionId) || typeof newText !== 'string') {
          sendJson(res, 400, { ok: false, error: 'invalid-request' });
          return;
        }
        if (newText.length === 0) { sendJson(res, 400, { ok: false, error: 'empty-text', message: '回复内容不能为空' }); return; }
        writeLog('info', 'assistant-edit', '收到助手回复改写请求', { sessionId, targetSeq, turn, messageId: messageId ?? null, chars: newText.length, hasOldText: typeof oldText === 'string' });
        const result = await editAssistantText(ctx, writeLog, {
          sessionId,
          targetSeq: typeof targetSeq === 'number' && Number.isFinite(targetSeq) ? targetSeq : undefined,
          turn: typeof turn === 'number' && Number.isFinite(turn) ? turn : undefined,
          messageId: typeof messageId === 'string' && messageId.length > 0 ? messageId : undefined,
        ...
        const code = err?.code ?? 'internal';
        writeLog('error', 'assistant-edit', '/bubble/edit-assistant 失败', { code, message: String(err?.message ?? err) });
        sendJson(res, 500, { ok: false, error: code, message: String(err?.message ?? err) });
      }
    }
  }));
  // 启动自愈：把"未归属任何工作区、但父会话属于某工作区"的分支会话挂回去。
  // 修复早期版本建分支时挂载失败留下的未分组子会话（也兜住以后任何漏挂）。
  void (async () => {
    try {
      let registry = null;
      try { registry = ctx.workspaceRegistry ?? (typeof ctx.get === 'function' ? ctx.get('workspaceRegistry') : null); } catch { registry = null; }
      const persistence = ctx.get('sessionPersistence');
      if (registry === null || registry === undefined || typeof registry.list !== 'function') return;
      ...
      for (const workspace of workspaces) for (const id of memberIds(workspace)) accounted.add(id);
      const headers = await persistence.list();
      let repaired = 0;
      for (const header of headers ?? []) {
        const id = header?.id;
        const parentId = header?.parentSession;
        if (typeof id !== 'string' || typeof parentId !== 'string' || accounted.has(id)) continue;
        const owner = workspaces.find((workspace) => memberIds(workspace).includes(parentId));
        ...
        accounted.add(id);
        repaired += 1;
        writeLog('info', 'assistant-branch', '启动自愈：未分组的分支会话已挂回工作区', { sessionId: id, parent: parentId, workspaceId: owner.id });
      }
      if (repaired > 0) writeLog('info', 'assistant-branch', '启动自愈完成', { repaired });
    } catch (err) {
      writeLog('warn', 'assistant-branch', '启动自愈失败（忽略）', { err: String(err?.message ?? err) });
    }
  })();
    ...
    handler: async (req, res) => {
      try {
        ...
        if (UPDATE_CHANNEL === 'none') {
          // 组合版：无更新通道。只回报当前版本，避免"假装能升级"或误升上游包。
          writeLog('info', 'host', 'check-update（无更新通道）', { current, dshVersion });
          sendJson(res, 200, { ok: true, current, latest: null, dshVersion, channel: 'none', packageName: OWN_PACKAGE_NAME });
          return;
        }
  ...
  disposers.push(ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const body = await readJsonBody(req, 16 * 1024);
        if (UPDATE_CHANNEL === 'none') {
          writeLog('warn', 'host', 'update-plugin：本构建无更新通道（已拒绝）', null);
          sendJson(res, 200, { ok: false, error: 'no-update-channel', message: '本插件为本地组合构建，无 npm 更新通道：改 parts/ 下源码 → 跑 build.mjs → 同步部署副本（或重新 dsh plugin add）后重启 DSH。' });
          return;
        }
        ...
        const output = await new Promise((resolve) => {
          // WM 分叉：包名已改为 -wm，绝不能再 up 上游包名（会把分叉覆盖掉）
          execFile('pnpm', ['up', 'dsh-message-recall-wm'], { cwd: profileDir, timeout: 90000, windowsHide: true }, (err, stdout, stderr) => {
